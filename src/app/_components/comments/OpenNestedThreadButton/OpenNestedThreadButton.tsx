...
import type { CommentGroupByIndex } from "@/models";
...
const OpenNestedThreadButton: React.FC<OpenNestedThreadButtonProps> =
  React.memo(
    ({
      ...
      const [isLoadingNestedComments, setIsLoadingNestedComments] =
      ...
      const calculatePositions = () => {
        ...
        const childPositionsArray = Object.values(comment_object || {}).map(
          (comment) => {
      ...
      return ReactDOM.createPortal(
        <div>
          ...
          {!isCollapsed && positions.verticalLinePositions && (
            <div
              style={{
                ...
                left: positions.verticalLinePositions.left,
                height:
                  positions.verticalLinePositions.toggleToBottomHeight - 25,
                width: "2px",
                backgroundColor: "var(--color-comment-font-detail)",
                transform: "translateX(-50%)",
              }}
          ...
          {isLoadingNestedComments && positions.verticalLinePositions && (
            <div
              style={{
                position: "absolute",
                top:
                  positions.verticalLinePositions.bottomStartPosition +
                  positions.verticalLinePositions.toggleToBottomHeight +
                  25,
                left: positions.verticalLinePositions.left + 25,
              }}
            >
              <LoadingOverlay sideLength={25} />
            </div>
          )}
          {!isCollapsed &&
            positions.childPositionsArray &&
          ...
          {!isCollapsed &&
            renderChildComments &&
            comment_object &&
            renderChildComments(comment_object)}
        </div>,
        commentContainerRef.current
      );
    }
